...
struct
  ...
  let create scope (i : _ I.t) : _ O.t =
    ...
    let wr_addr_gen =
      reg_fb
      spec
      ~enable:conv2d_o.conv.valid
      ~width
      ~f:(mod_counter ~max:(P.size - 1))
      -- "wr_addr_gen"
    in
    let wr_port =
    ...
    in
    let feed_en = (*if rd_addr = 0 then start_counting else 1*)
    ...
    let conv2d_buf =
      multiport_memory 
        P.size
        ~write_ports:[|wr_port|]
        ~read_addresses:[|rd_addr_gen|]
    in
    let ans_valid = reg spec
      (conv2d_o.ans ==: ans) &: (conv2d_o.o_last &: conv2d_o.conv.valid)
    in
  ...
  let hierarchical scope =
    ...
    Scoped.hierarchical ~scope:scope ~name:"erode" create
end
