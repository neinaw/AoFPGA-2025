...
struct
  ...
  let create _scope (i : _ I.t) : _ O.t =
    ...
    let conv_map =
      List.init P.size ~f:(fun i ->
        let get_bit row idx = if idx < 0 || idx >= P.size then gnd else row.:[idx, idx] in
        let neighbourhood =
          [ get_bit line_buf_zz (i - 1)
          ; get_bit line_buf_zz i
          ; get_bit line_buf_zz (i + 1)
          ; get_bit line_buf_z (i - 1)
          ; get_bit line_buf_z (i + 1)
          ; get_bit din_padded (i - 1)
          ; get_bit din_padded i
          ; get_bit din_padded (i + 1)
          ]
        in
    ...
    let ans =
      reg_fb spec ~width:16 ~f:(fun d -> uresize reachables_popcount ~width:16 +: d)
    in
    { O.conv = { value = o_buf_pipe; valid = valid_pipe3 }; O.o_last = last_pipe3; O.ans }
  ;;
