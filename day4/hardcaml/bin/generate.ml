...
let generate_rtl_conv2d () =
  let module Conv2d = Solution.Conv2d.Make(struct
    let size = size
  end) in
...
let generate_rtl_erode () =
  let module Erode = Solution.Erode.Make(struct
    let size = size
  end) in
...
let () =
  Command_unix.run
    (Command.group ~summary:"RTL Generation tools"
    [ "conv2d", rtl_command_conv2d
    ; "erode", rtl_command_erode
    ])
;;
