...
let create scope (i : _ I.t) : _ O.t =
  ...
  let product = srl (magic_number *: srl i.din ~by:sh_pre) ~by:(_N - e + sh_post) in
  let quo = reg spec (uresize product ~width:16) in
  let quo_pipe = reg spec quo in
  let rem =
    reg spec (uresize (din_pipe -: uresize (quo *: divisor) ~width:16) ~width:8)
  in
  Always.(
    compile
      [ when_
          (valid_pipe ==:. 1)
          [ if_
              (sign_pipe ==:. 0) (*Rotating Right*)
              [ if_
                  (rem +: pos.value >:. 100)
                  [ pos <-- rem +: pos.value -:. 100 ]
                  [ if_
                      (rem +: pos.value ==:. 100 |: (rem +: pos.value ==:. 0))
                      (*if din= 0 mod 100 and pos = 0*)
                      [ p1_count <-- p1_count.value +:. 1; pos <--. 0 ]
                      [ pos <-- rem +: pos.value ]
                  ]
              ]
              [ (*Rotating Left*)
                if_
                  (rem >: pos.value)
                  [ pos <-- pos.value +:. 100 -: rem
                  ; p2_count <-- p2_count.value +: quo_pipe +:. 1
                  ]
                  [ if_
                      (rem ==: pos.value)
                      [ pos <--. 0; p1_count <-- p1_count.value +:. 1 ]
                      [ pos <-- pos.value -: rem ]
                  ]
              ]
